...
Если в удаленном репозитории были внесены какие-либо изменения, мы можем выкачать актуальное состояние и слить изменения из этого репозитория с помощью команды:
```
git pull
```
Если нужно просто звгрузить изменения без слияния, то можно воспользоваться командой:
```
git fetch
```
### Pull Request
Команда *`pull request`* служит для предложения изменений. С помощью нее мы можем отправить запрос на вливание изменений в чужой репозиторий.

Чтобы сделать ***pull request***, для начала нужно в своем аккаунте на GitHub создать копию репозитория из другого аккаунта с помощью кнопки **Fork**.
Затем мы клонируем удаленный репозиторий на свой компьютер с помощью команды:
```
git clone <URL-адрес удаленного репозитория>
```
После этого нужно создать новую ветку и уже в нее вносить все изменения. Обязательно закоммитить все изменения.

Отправляем свою версию в свой аккаунт GitHub с помощью команды:
```
git push
```
На сайте появится кнопка **pull request**, с помощью нее и отправляем все автору данного репозитория.
